...
export function metaFor(name: string): CountryMeta {
  ...
  if (!meta) {
    throw new Error(
      `Unknown country ${
        JSON.stringify(name)
      } in traveling.yaml — add it to _lib/country-codes.ts`,
    );
  }
  return meta;
}
